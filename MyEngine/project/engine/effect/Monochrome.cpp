...
void Monochrome::Initialize(DirectXCommon* dxCommon, SrvManager* srvManager) {
	dxCommon_ = dxCommon;
	srvManager_ = srvManager;
	CreateGraphicsPipelineState();

	paramResource_ = dxCommon_->CreateBufferResource(sizeof(MonochromeParam));
	paramResource_->Map(0, nullptr, reinterpret_cast<void**>(&paramData_));

	//paramData_->color = { 1.0f, 74.0f / 107.0f, 43.0f / 107.0f }; // セピア調
	paramData_->color = { 1.0f,1.0f,1.0f }; // Grayscale
	paramData_->intensity = 1.0f;

}

void Monochrome::Draw(uint32_t srcSrvIndex) {
...
void Monochrome::DrawImGui() {
#ifdef USE_IMGUI
	// 色を編集（paramData_に書けば即反映）
	...
	if (useGrayscale) {
		paramData_->color = { 1.0f, 1.0f, 1.0f };
	}
	bool useSepia = false;
	ImGui::Checkbox("Sepia", &useSepia);
	if (useSepia) {
		paramData_->color = { 1.0f, 74.0f / 107.0f, 43.0f / 107.0f };
	}
#endif
}

void Monochrome::CreateRootSignature() {
...
void Monochrome::CreateGraphicsPipelineState() {
	CreateRootSignature();
	...
	// VSはFullscreen（共通）、PSはMonochrome
	Microsoft::WRL::ComPtr<IDxcBlob> vertexShaderBlob =
		dxCommon_->CompileShader(L"resources/shaders/Fullscreen.VS.hlsl", L"vs_6_0");
	assert(vertexShaderBlob != nullptr);
	Microsoft::WRL::ComPtr<IDxcBlob> pixelShaderBlob =
		dxCommon_->CompileShader(L"resources/shaders/Monochrome.PS.hlsl", L"ps_6_0");
